...
| B12 | ~~MFA/aal2 no super-admin~~ **código protegido (2026-09-08)** | As leituras globais do `/admin` agora exigem AAL2 antes de qualquer consulta service-role (métricas, usuários, feedback, administradores, auditoria e detalhe); ações sensíveis já tinham o mesmo gate. O painel não carrega dados com AAL1 e oferece ativação/confirmação do TOTP | `requireAdmin` + `exigir(..., { aal2: true })`, gate visual e `test:admin-governanca` (34 checks) | Ainda cadastrar/verificar um TOTP na conta operacional `ADMIN_EMAIL` no Supabase Auth e repetir o fluxo com uma sessão real | `/admin` com MFA inscrito e smoke autenticado concluído |
