...
- [ ] **Skeleton infinito na Home** — `src/screens/HomeScreen.tsx:206-224` `load()` sem try/catch: qualquer falha nas 8 queries trava `carregando=true` pra sempre + trava o pull-to-refresh (`refresh()` sem catch). Tela mais usada. Abordagem: envolver em try/catch com estado de erro+retry (o `loadCobranca` no MESMO arquivo já faz certo — copiar padrão).
...
### Segurança
- [ ] **XSS armazenado no PDF do orçamento** — `pdfGenerator.ts` (modeloPdf) não passa por `modeloSeguro()`; o recibo já foi corrigido com esse mesmo helper. Abordagem: reaplicar `modeloSeguro()` ao orçamento.
...
### Painel (webapp/)
- [ ] **Links de filtro KPI mortos** — `pages/olli/inicio/financeiro.ts:paramStatus` gera `/orcamentos?status=...` mas `pages/olli/orcamentos/index.tsx` nunca lê `useSearchParams` → clicar em "Em jogo"/"A receber" abre lista sem filtro. Abordagem: OrcamentosPage lê `?status=` no mount.

### Landing (web/)
- [ ] **FAQ mente e vira JSON-LD pro Google** — `web/src/pages/index.astro:163-164` afirma "é o mesmo sistema no app do celular e no navegador" — falso (`llms.txt.ts`: APK não publicado; painel web não tem voz/offline). Entra no schema.org FAQPage. Abordagem: alinhar à verdade (navegador celular+PC hoje; app Android a caminho), derivando de `llms.txt`.
...
- [ ] **404 sem noindex + CTA errado** — `web/src/pages/404.astro`: sem `<meta robots noindex>` (Layout.astro nem tem a prop) e CTA "Ir para o painel" usa `APP_URL` (login) em vez de `APP_CADASTRO_URL` (quebra CC-01). Abordagem: prop noindex no Layout + usar cadastro.

## P1 — coerência / UX / venda
- [ ] **Feature paga invisível: EquipeAoVivoScreen** — `src/screens/EquipeAoVivoScreen.tsx` 100% pronta, roteada (`/equipe/ao-vivo`), gateada por `GatePro` (Empresa) desde 16/07, mas NENHUM botão leva até ela (só URL). E `PlanosScreen.tsx:125-126` + Landing/FAQ ainda vendem "Equipe ao vivo no mapa (em breve)". Abordagem: entry-point em EquipeScreen + EquipeDesktopScreen (respeitando gate) + corrigir copy "em breve".
