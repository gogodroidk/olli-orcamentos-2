...
- **Onda 4 — Segurança & dado que some & coerência** (próxima): (B1) worker: teto de payload + rate-limit por IP em /stripe/webhook e /transcrever; (B2) Sinal (R$+data) + Laudo técnico aparecerem no PDF do orçamento (hoje o prestador digita e some); (B3) painel: Diagnóstico IA com gate por vertical (é HVAC-only sem gate) + redirect /privacidade→/legal/privacidade (Play valida a URL); (B4) fechar reduced-motion (OlliSkeleton, mic, "digitando", wizard). Fan-out disjunto.
