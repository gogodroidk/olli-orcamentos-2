...
- **Onda 7 — Correção visual (bugs objetivos)** (próxima): F1 contraste app (OlliButton cloneElement + StepIndicator + categorias + voice/plan), F2 landing polish (hover, verde token, toque CTA, ＋→+, aria), F3 app-desktop (PainelNovoPlano→Modal, largura 460→420, CTA Agenda), F4 painel unifica ícone (Solar→lucide). A identidade cross fica pro dono decidir.
