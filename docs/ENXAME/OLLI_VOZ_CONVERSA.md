...
- **Fase 2 — cobrança real** ✅ (2026-07-17, Wave 15): `cobrarCreditoVoz` no worker cobra 1 crédito nas rotas /voz e /transcrever(orçamento) **SÓ com `confirmarCredito:true`** (nunca auto-débito), idempotente (ref explícito ou hash do conteúdo), bloqueia só em saldo zero, **fail-open na infra**. App: gate gracioso (usar 1 crédito / ver planos / montar na mão). Teste `teste-creditos-voz.ts` (29 asserts) na cadeia; typecheck+test verdes. **⚠️ 2 passos HUMANOS antes de valer:** (1) `wrangler deploy` do worker; (2) fechar 100% o vazamento do grátis (3/mês ainda client-side/burlável) exige **migration nova** de contagem por período — follow-up documentado no código (`// FOLLOWUP` em creditos.js).
- **Fase 3 — Tier B (a conversa de verdade)** ✅ (2026-07-18, Wave 16): a Olli **pergunta o que falta** e o prestador responde até montar. Worker `handleVozConversa` (`/voz/conversa`, novo `worker/src/voz.js`; `gemini`/`util` extraídos do index) devolve `{pergunta}` ou `{pronto:true, itens}`, multi-turno via `gemini()` (histórico), teto de 6 turnos **enforçado no código**, `fechar:true` = "montar com o que tem". Cobra **1 crédito por CONVERSA** (idempotente por `conversationId`; turnos de pergunta não cobram). App: toggle "Conversar" no `OlliVozScreen` (off por padrão), bolhas estilo OlliChat, termina na Revisão (nada salva sozinho). Teste `teste-voz-conversa.ts` (33 asserts); typecheck+test verdes. TTS (Olli falar) e casar-cliente = follow-up. ⚠️ precisa `wrangler deploy` (humano).
