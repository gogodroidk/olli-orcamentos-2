...
| 2026-07-17T03:13:37Z | F0d | FIM DONE | f840e95 — DECISAO TOMADA (dono delegou): grandfathering por flag. Org que ja existia mantem Equipe; org NOVA precisa do Empresa. Motivo: nao da p/ saber quantas contas reais seriam cortadas (o hook de guarda bloqueia MCP em loop e NAO contornei trava de seguranca) -> sob incerteza escolhe-se o REVERSIVEL (desfaz com 1 UPDATE; usuario cortado churna e nao volta, justo as vesperas de cobrar dele). Plano-Mestre linha 529 aponta o mesmo. Entregue: migration 20260725 (coluna default FALSE + backfill true no instante da aplicacao) + enforcement no worker (orgTemEmpresaAtivo devolve sim ANTES de olhar o plano) + entitlementEquipe.ts (regra 11.2: "PODE X", nunca "E Empresa") + GateEquipe.tsx nas 3 telas (indeterminado NAO mostra muro: nao acusa por erro de rede). PROVA: 11 asserts exit 0 (incl. coluna ausente != acesso gratis); mutacao vaza -> exit 1; suite 100 asserts exit 0; typecheck exit 0. DONO: migration ANTES de publicar o worker. |
| 2026-07-17T03:16:43Z | FOLLOWUPS-29 | FIM DONE (parcial) | 01b560f — espelharVersaoNuvem com 3 estados: tenant desconhecido NAO espelha (antes gravava no tenant errado = versao orfa, invisivel ao dono, poluindo o tenant do tecnico). Estritamente melhor: o erro deixou de CRIAR linha errada. FALTA retry (orcamento_versoes nao esta no pipeline de sync) — item segue aberto. |
| 2026-07-17T03:16:43Z | O2-19 | NOTA | Recomendacao CORRIGIDA em FOLLOWUPS #31: a "opcao 4" sozinha NAO resolve — se so o painel usar RPC, o app continua no contador local e os dois colidem. E o seed do contador nao pode ser verificado (guarda bloqueia MCP; nao se contorna trava de seguranca). Segue BLOQUEADO ate o dono decidir E o seed poder ser conferido. |
