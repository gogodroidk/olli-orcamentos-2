...
| C7 | Integrações e central do prestador | BLOCKED_EXTERNAL | central oficial INSS/NFS-e/Sebrae, agenda/notificações/fallback e arquitetura por portas validados localmente; staging Supabase isolado recebeu baseline + 41 migrations, Storage privado foi comprovado e Worker staging está publicado em modo simulador; OAuth/Resend/WhatsApp Cloud/fiscal e smoke autenticado ainda aguardam secrets/escopos externos; evidência em `ACEITE_C7_INTEGRACOES_RECURSOS_2026-09-05.md` e `ops/staging/MIGRATION_CHECKPOINT.md` |
| C8 | IA operacional segura | DONE_LOCAL | chat gera somente rascunho estruturado; alvo único, RBAC, allowlist, token de confirmação, diff antes/depois, compare-before, journal, retenção de 30 dias e desfazer foram ligados no painel + Worker + migration; testes e build verdes. Aplicação da migration, deploy e canário real permanecem no gate C11; evidência em `ACEITE_C8_IA_SEGURA_2026-09-05.md` |
...
| C11 | Hardening e release | BLOCKED_EXTERNAL | base local revalidada em 2026-09-06 com `npm run preflight`, builds web/webapp, QA desktop/mobile, PWA, Worker dry-run, Semgrep, Gitleaks, contraste, Expo Doctor 20/20 e canary HTTP; promoção permanente foi versionada em `ops/`/`.github/workflows/promotion.yml`; staging recebeu baseline + 41 migrations, verificação metadata-only e Worker em `workers.dev` HTTP 200; histórico de migrations manual, secrets externos, smoke autenticado, revisão independente, observabilidade/restore, canário remoto, Git/Cloudflare, publicação e aceite do dono continuam gates externos; evidência em `ACEITE_C11_HARDENING_RELEASE_2026-09-05.md` e `ops/staging/MIGRATION_CHECKPOINT.md` |
