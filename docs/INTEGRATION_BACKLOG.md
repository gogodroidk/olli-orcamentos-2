...
**Fallback atual:** cada linha do backlog explicita a alternativa manual/local que
continua funcionando quando o provedor externo está ausente ou indisponível.
...
|---|---|---|---|---|---|---|---|---|
| SUB | `SubscriptionProvider` | Receita do SaaS: prestador assina Pro/Empresa (mensal/anual/12x) e gerencia no portal. | **Stripe** (Checkout + Portal); webhook = fonte da verdade do plano (D-03). | `PlanosScreen.tsx` + `worker/src/stripe.js` + `planos.ts` (lê `assinaturas`). Runtime e conta existem; Prices/webhook live ainda exigem reconciliação comprovada. | Plano cai no cache local por 7 dias de graça (`planos.ts`); nunca bloqueia quem pagou e ficou offline. | Confirmar installments, Prices/lookup keys e webhook no modo live antes de vender. | Onda 1 | **P0** |
| AI | `AiProvider` | O diferencial da OLLI: diagnóstico por código de erro, orçamento por voz/texto, chat e ações assistidas. | Worker `olli-diagnostico` com OpenRouter/Workers AI; segredo só no Worker. | Diagnóstico/voz/chat existentes continuam operantes. Ações assistidas têm UI + Worker + migration local com rascunho, RBAC, confirmação, auditoria e rollback; ainda não foram promovidas. | Cache local + base offline para códigos; ação sem contexto, permissão, banco ou confirmação falha fechada e não altera nada. | Aplicar migration e executar canário em staging antes de publicar as novas rotas. | C8/C11 | **P0** |
...
| EMAIL | `EmailProvider` | Entregar orçamento/recibo/boas-vindas/convite por e-mail. | **Resend**, remetente no domínio raiz verificado `olliorcamentos.online`. | Outbox, adaptador transacional, idempotência e suppressions implementados; dispatch continua em `simulator`. A rota local `/resend/webhook` agora valida Svix, remove PII e registra idempotência; no painel Resend ainda não há webhook e o template está em rascunho. | Deep-link `mailto:`/WhatsApp e consumer simulador; nenhum destinatário real sai sem promoção deliberada. | Registrar webhook assinado no Resend, gravar `RESEND_WEBHOOK_SECRET` no cofre, publicar templates e promover dispatch após canário autorizado. | Onda 6 | **P1** |
| STORAGE | `StorageProvider` | Logo/fotos/PDF com URL consistente em web+mobile+e-mail. | **Supabase Storage** privado com RLS por tenant visível. | Adaptador `SupabaseStorageProvider` + migration de buckets/policies implementados localmente; call-sites ainda preservam URI local até migração gradual. | URI local continua funcionando; upload remoto falha fechado sem perder o arquivo do aparelho. | Aplicar primeiro em staging e validar RLS/restore; conector atual aponta para projeto diferente do app. | Onda 7 (PDF v2) | **P1** |
