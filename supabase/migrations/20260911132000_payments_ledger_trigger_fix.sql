-- Corrige a trigger do ledger para não comparar OLD no caminho INSERT.
-- A versão original já foi aplicada no staging; esta substituição é aditiva e
-- mantém a proteção append-only para UPDATE/DELETE.
